...
const Products = () => {
  ...
  const siteUrl = "https://chorvador.uz";
  ...
  const seoData = useMemo(() => {
    ...
    const baseDescription =
      language === "uz"
        ? "Chorvador.uz saytida chorvachilik va qishloq xo‘jaligi uchun mahsulotlar katalogi."
        : language === "ru"
          ? "Каталог товаров для животноводства и сельского хозяйства на Chorvador.uz."
          : "Product catalog for livestock and agriculture on Chorvador.uz.";
    ...
    if (selectedCategory !== "all") {
      ...
      if (categoryName) {
        ...
        pageDescription =
          language === "uz"
            ? `${categoryName} kategoriyasidagi mahsulotlar bilan tanishing.`
            : language === "ru"
              ? `Ознакомьтесь с товарами в категории ${categoryName}.`
    ...
    if (searchQuery.trim()) {
      pageTitle =
        language === "uz"
          ? `"${searchQuery}" bo‘yicha mahsulotlar`
          : language === "ru"
      ...
      pageDescription =
        language === "uz"
          ? `"${searchQuery}" so‘rovi bo‘yicha topilgan mahsulotlar.`
          : language === "ru"
            ? `Товары, найденные по запросу "${searchQuery}".`
            : `Products found for the search query "${searchQuery}".`;
    }
    ...
    const currentUrl = `${siteUrl}/products${
  ...
  }, [language, selectedCategory, searchQuery, currentPage, categories]);
  ...
  return (
    <Layout>
      <Helmet>
        ...
        <meta
          property="og:title"
          content={`${seoData.pageTitle} | ${siteName}`}
        />
        ...
        <div className="container mx-auto px-4 sm:px-6 lg:px-8 max-w-7xl">
          ...
          <div className="flex flex-col sm:flex-row gap-3 sm:gap-4 mb-6 sm:mb-8">
            <div className="relative flex-1">
              <Search
                className="absolute left-3 sm:left-4 top-1/2 -translate-y-1/2 w-4 h-4 sm:w-5 sm:h-5 text-muted-foreground pointer-events-none"
                aria-hidden="true"
              />
              <Input
          ...
          {loading && (
            ...
            >
              <Loader2
                className="w-10 h-10 sm:w-12 sm:h-12 animate-spin text-primary mb-4"
                aria-hidden="true"
              />
          ...
          {error && (
            <div
              className="bg-destructive/10 border border-destructive/20 rounded-lg sm:rounded-xl p-6 sm:p-8 text-center"
              role="alert"
              aria-live="assertive"
            >
          ...
          {!loading && !error && totalCount > 0 && (
            <div
              id="products-grid"
              className="flex items-center gap-2 text-muted-foreground mb-4 sm:mb-6"
              role="status"
              aria-live="polite"
            >
